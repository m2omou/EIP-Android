...
public class MapView extends FragmentActivity implements LocationListener{
	...
	public int limit = 15;    //  def 10
	public int radius = 200;    //  def 800
	public int bufferPlace = 15;   //  def  20
	...
	private void GetCurrentLocation() {
	    ...
	    //gMap.addMarker(new MarkerOptions().position(new LatLng(d[0], d[1])).title("Current Location").icon(BitmapDescriptorFactory.fromResource(R.drawable.new_menu)));
	    ...
	    gMap.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(d[0], d[1]), 17));
...
//-------------------------------------------------------------------------
	...
	///////////////////////////////////////////////////////////////////////////////////////
        ...
        //posMarker = gMap.addMarker(new MarkerOptions().title("Vous étes ici").position(new LatLng(45.75, -0.633333)));
        ...
        changeFilter = new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem arg0) {
				...
				final CharSequence[] items = new CharSequence[categories.list.length + 1]; //  = {"Autoriser les autres utilisateurs a me contacter"}//;, "Autoriser commentaires sur mes publications", "Autoriser commentaires sur mes messages"};
				//final boolean[] check = new Boolean[categories.list.length];    //new ArrayList<Boolean>();      // = {false}

                items[0] = "Aucun";

				for (int i = 1; i < categories.list.length + 1; i++) {
					items[i] = categories.list[i - 1].name;
					//check[i] = false;
				}
				...
				public void onClick(DialogInterface dialog, int which) {
					if (which == 0)
                        ...
                        categorieId = categories.list[which - 1].id;
							...
							builder.setItems(charSequenceArray, new DialogInterface.OnClickListener() {
								...
								@Override
									public void onClick(DialogInterface dialog, int item) {
							                ...
							                CameraPosition cp = new CameraPosition(new LatLng(searchPlaces.list[item].lat, searchPlaces.list[item].lon), 17, 0, 0);
							                //etLocation.setText("");						                
							                gMap.moveCamera(CameraUpdateFactory.newLatLngZoom(cp.target, 17));
    	...
    	{ 
    		...
    		{	
    			if (!isAlreadyHere(places.list[i]))
    			{
                    ...
                    if (places.list[i].publications > 0 && places.list[i].publications <= 5)
                        places.list[i].marker = gMap.addMarker(new MarkerOptions().title(places.list[i].name).position(new LatLng(places.list[i].lat, places.list[i].lon)).snippet(places.list[i].address).icon(BitmapDescriptorFactory.fromResource(R.drawable.pin_dark_green_2o)));
                    else if (places.list[i].publications > 5)
                        places.list[i].marker = gMap.addMarker(new MarkerOptions().title(places.list[i].name).position(new LatLng(places.list[i].lat, places.list[i].lon)).snippet(places.list[i].address).icon(BitmapDescriptorFactory.fromResource(R.drawable.pin_dark_green_2g)));
                    else
                        places.list[i].marker = gMap.addMarker(new MarkerOptions().title(places.list[i].name).position(new LatLng(places.list[i].lat, places.list[i].lon)).snippet(places.list[i].address).icon(BitmapDescriptorFactory.fromResource(R.drawable.pin_dark_green_2r)));
